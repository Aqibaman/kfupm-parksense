...
export function FloorSummaryCards({ floors }: { floors: FloorSlotGridResult[] }) {
  return (
    <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-5">
      {floors.map((floor) => (
        <div key={floor.floor.key} className="rounded-2xl border border-[#dbe9e1] bg-[#f2fbf5] p-4">
          ...
          <div className="mt-4 grid grid-cols-3 gap-2 text-center">
            <div className="min-w-0 rounded-xl bg-white px-1.5 py-2">
              <p className="text-[10px] leading-4 text-slate-400">Vacant</p>
              <p className="text-sm font-semibold text-emerald-700">{floor.availableCount}</p>
            </div>
            <div className="min-w-0 rounded-xl bg-white px-1.5 py-2">
              <p className="text-[10px] leading-4 text-slate-400">Occupied</p>
              <p className="text-sm font-semibold text-rose-700">{floor.occupiedCount}</p>
            </div>
            <div className="min-w-0 rounded-xl bg-white px-1.5 py-2">
              <p className="text-[10px] leading-4 text-slate-400">Unavailable</p>
